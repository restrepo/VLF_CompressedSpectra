...
#include <sstream>
#include <fstream>
#ifndef _sumsignalevents_H_
#define _sumsignalevents_H_
using namespace std;
#endif
...
int main(int argc, char *argv[])
{
 ...
 for (int i=1;i<6;i++) {
   ...
   temp<<i;
   sint=temp.str();
   filename= "signal/results/signal150145"+sint+".txt"; //putting together filename     
 ...
 cout<<" ndt ="<<ndt<<endl; 
 datostotal.open("signal150145total.dat");
 for (int i=0;i<ndt;i++){
   for (int j=0;j<16;j++){
     datostotal<<scan[i][j]<<" ";
