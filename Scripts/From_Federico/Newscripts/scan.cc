...
// emulate new trigger conditions: PT_mu> 5 GeV, JetPT>60 GeV,  MET> 60 GeV


        
     if(NumMuons==1&&Numjets>0){

//      if (leadingjetpt<=60.0||metpointer->MET<=60.0||muonpointer->PT<=5.0) continue;
      cutflow[1]++;  
      cutflow[2]++;     
      cutflow[3]++;
	...
	for (int i1=0;i1<nmuonpt;i1++){ //loop over pt cuts
	  if (muonpointer->PT<muonptcut[i1]) {
	    scan1[i1]++;
	    for (int i2=0;i2<nmt;i2++){ //loop over mt cuts
	      if (mt_mumet<mtcut[i2]) {
		scan2[i1][i2]++;
		for (int i3=0;i3<nljetpt;i3++){ //loop leading jet pt cuts
		  if (leadingjetpt>ljetptcut[i3]) {
		    scan3[i1][i2][i3]++;
		    for (int i4=0;i4<nmet;i4++){ //loop over met cuts
		      if (metpointer->MET>metcut[i4]) {
			scan4[i1][i2][i3][i4]++;
			for (int i5=0;i5<njets;i5++){ //loop over njet cuts
			    if (Numjets<njetscut[i5]) {
			    scan5[i1][i2][i3][i4][i5]++;
			    for (int i6=0;i6<ndphi;i6++){ //loop over dphi cuts
			      if (fabs(dphi_mumet-pi)>=dphicut[i6]) {
				scan6[i1][i2][i3][i4][i5][i6]++;
				outfile1<<entry<<" "<<muonpointer->PT<<" "<<muonpointer->Phi<<" "<<metpointer->MET<<" "<<metpointer->Phi<<" "<<Numjets<<" "<<leadingjetpt<<" "<<mht<<endl;
			      } //if of dphi cut6
			    } //loop over dphi cut
			  } //if of njets cut5
			} //loop over met cut		       
